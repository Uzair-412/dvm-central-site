...
class EventsTable extends DataTableComponent
{
    ...
    {
        return Events::query()->when($this->getFilter('search'), fn ($query, $term) => $query->where('id', 'like', '%'.$term.'%')
        ->orWhere('name', 'like', '%'.$term.'%')->orWhere('start_date', 'like', '%'.$term.'%')
        ->orWhere('end_date', 'like', '%'.$term.'%')->orWhere('type', 'like', '%'.$term.'%'))
    ...
    {
        return 'backend.events.includes.row';
    }
}
