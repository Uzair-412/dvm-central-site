...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if($user && Hash::check($request->password,$user->password))
        {
            ...
            {
                return response()->json(['is_verified' => false,'error' => 'Your email is not verified!'], 200);
            }
            ...
            return response()->json(['is_verified' => true, 'user' => $user, 'token' => $token], 200);
        }
        else
        {
            return response()->json(['error' => 'These credentials do not match our records.'], 402);
        }
        ...
        //     if(@$request->device_token && $request->type)
        //     {
        //         $this->store_device_token(['device_token'=>$request->device_token, 'type'=>$request->type],auth()->user()->id);
        //     }
        //     $user = auth()->user();
        //     $token = $user->createToken('VetandTechToken')->accessToken;
