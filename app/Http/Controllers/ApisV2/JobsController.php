...
class JobsController extends Controller
{
    public function listing()
    {
        ...
        $data['job_detail'] = VendorJob::where('application_end_time', '>=', time())->with(['vendor:id,name'])->with(['country:id,name'])->with(['state:id,name'])->with(['salary_type_:id,name'])->with('job_working_time')->with(['minimum_education_level:id,name'])->paginate(12);
        ...
        foreach ($data['job_detail'] as $job_detail) {
            ...
            $data['filters']['states'] = State::join('vendor_jobs', 'vendor_jobs.state_id', '=', 'states.id')
            ->select('states.id','states.name')
            ->groupBy('states.id')
            ->get();
             
            $data['filters']['country'] = Country::join('vendor_jobs', 'vendor_jobs.country_id', '=', 'countries.id')
            ->select('countries.id','countries.name')
            ->groupBy('countries.id')
            ->get();
            $data['filters']['company'] = Vendor::join('vendor_jobs', 'vendor_jobs.vendor_id', '=', 'vendors.id')
            ->select('vendors.id','vendors.name')
            ->groupBy('vendors.id')
            ->get();
            $data['filters']['max_price'] = VendorJob::max('salary');
            $data['filters']['min_price'] = VendorJob::min('salary');
            ...
            // $job_detail->vendor->email =Customer::getCustomerEmail($job_detail->vendor->user);
            // $data['filters']['states'] = $job_detail->state->name;
        }
    
        $data['filters']['working_times'] = JobWorkingTime::all();
        $data['filters']['categories'] = JobCategory::all();
        $data['filters']['types'] = JobType::all();
        $data['filters']['salary_types'] = SalaryType::all();
        $data['filters']['education_levels'] = EducationLevel::all();
    ...
    public function apply(Request $request)
    {
        ...
        return response()->json(['success' => 'Job apllied successfully'], 200);
    }
    ...
    public function addToWishlist($jobId, $userId)
    {
        // getting wishlist record against active user
        $wishlist_record = JobWishlist::where('user_id', $userId)
            ->where('job_id', $jobId)
            ->first();
            
        // if wishlist record exist we will delete it orelse the record will be created
        if(!$wishlist_record) {
            $wishlist = new JobWishlist();
            $wishlist->job_id = $jobId;
            $wishlist->user_id = $userId;
            $wishlist->save();
            return response()->json(['success' => 'Job Added To Wishlist Successfully'], 200);
        }else{
            $wishlist = JobWishlist::find($wishlist_record->id);
            $wishlist->delete();
            return response()->json(['success' => 'Job Removed From Wishlist Successfully'], 200);
        }
    }
}