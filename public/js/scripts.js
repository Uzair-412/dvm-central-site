...
function open_video_modal(video,video_type) {
	if(video_type == 'file') {
		video = '/up_data/products/videos/' + video;
		let video_tag = '<video width="760" controls><source src="' + video + '" id="product_video" type="video/mp4">Your browser does not support the video tag.</video>';
		$('#video_modal').html(video_tag);
		$('#product_video_modal').modal('show');
	}else if(video_type == 'youtube'){
  	    video = "https://www.youtube.com/embed/" + video;
		let video_tag =
		'<iframe src = "' + video +'" width="760px" height="450px"></iframe>';
		$("#video_modal").html(video_tag);
		$("#product_video_modal").modal('show');
...
function modify_product_variation(id) {
	...
	$.ajax({
		...
		if (obj.status == '1') {
			...
			$('#sku').val(prd.sku);
			$('#wholesale_price').val(prd.wholesale_price);
...
function save_slugs(cbk) {
	...
	if (cbk.checked) $('#link_generate_slug').removeClass('d-none');
	else $('#link_generate_slug').addClass('d-none');
}
