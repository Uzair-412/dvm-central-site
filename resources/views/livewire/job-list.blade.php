<div>
    <div class="heading-wrapper flex justify-between items-center">
        <p class="text-xl font-bold">Job Listings @if ($edit_mode) {{ $counter }} @endif</p>
        <p @click="sb_open('edit_jobs')" id="add_edit_job_link"></p>
        @if ($edit_mode && $total_jobs < $max_jobs)
            <p @click="openModal('modal-overlay')" wire:click="$emit('addJob')" class="cursor-pointer text-sm primary-color">Add</p>
        @endif
    </div>
        ...
        @foreach ($jobs as $job)
            ...
            <div class="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden">
                <div class="flex items-end justify-end h-56 w-full bg-cover"
                    ...
                    <span wire:click="open_job({{ $job->id }})" class="@if ($edit_mode) -mb-3 @else -mb-4 @endif bg-blue-400 cursor-pointer hover:bg-blue-500 mx-2 rounded text-center text-white @if ($edit_mode) w-6 h-6 @else w-8 p-1  @endif">
                        <i class="fa fa-eye"></i>
                    </span>
                    @if ($edit_mode)
                        <span @click="openModal('modal-overlay')" wire:click="$emit('editJob', {{ $job->id }})"
                        ...
                        <span wire:click="destroy({{ $job->id }})"
                ...
                <div class="px-5 py-3">
                    ...
                    <span class="text-gray-500 mt-2">
                        {{ \App\Models\EvJob::$categories[$job->category_id] }}
                    </span>
                </div>
            </div>
