...
@push('head-area')
    ...
    <!--<link rel="stylesheet" type="text/css" href="static/css/stripe-elements.css" data-rel-css="" />-->
@endpush
@section('content')
    @livewire('frontend.cart',['rand_num' => session()->get('rand_num'),'checkout' => true,'v_shipping_details'=>@$data['v_shipping_details']])
    ...
    <div class="remove-from-cart-pop-container fixed top-0 left-0 w-screen h-screen z-50 flex justify-center items-center hidden opacity-0">
        <div class="remove-from-cart-pop-wrapper flex flex-col justify-center items-center bg-white py-6 px-6 sm:px-20 opacity-0">
            ...
            <div class="remove-from-cart-pop-btns-wrapper flex justify-center items-center flex-col sm:flex-row mt-6">
                <button class="cancel-btn btn blue-btn lite-blue-bg-color text-white z-10 py-2 md:py-3 px-4 md:px-6 overflow-hidden relative block w-max text-center">Cancel</button>
                <button class="remove-btn btn red-bg red-btn text-white z-10 py-2 md:py-3 px-4 md:px-6 overflow-hidden relative block w-max sm:ml-4 mt-2 sm:mt-0">Remove</button>
            </div>
        </div>
    </div>
@endsection
...
@push('after-scripts')
    <script defer src="{{ asset('assets/js/checkout.js') }}"></script>
