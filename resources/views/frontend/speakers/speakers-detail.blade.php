...
@section('content')
    ...
    @endphp
    <section class="block overflow-hidden w-full" style="height: 500px;">
        <div class="w-full h-full bg-center bg-cover" style="background-image: linear-gradient(0deg, rgb(249, 250, 251), rgb(249, 250, 251), rgba(255, 255, 255, 0.7), #fbfbfb00, #fbfbfb00,#fbfbfb00), url('https://images.unsplash.com/photo-1499336315816-097655dcfbda?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=crop&amp;w=2710&amp;q=80');">
    ...
    <div class="width content-container pt-6 mx-auto xl:px-10 pb-10 -mt-40">
        <main class="profile-page">
            <section class="relative overflow-visible">
                <div class="container mx-auto px-4">
                    <div class="relative flex flex-col min-w-0 break-words w-full mb-6 bg-white p-4 shadow">
                        <div class="px-6">
                            <div class="flex flex-wrap justify-center -mt-24">
                                <div class="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
                                    <img alt="..." src="{{ $path }}" class="w-40 h-40 shadow-xl rounded-full h-auto align-middle border" />
                                </div>
                            ...
                            <div class="text-center mt-4">
                                ...
                                <div class="text-blueGray-600">
                                    ...
                                    Desgination: {{ $data['speaker_data']->job_title }}
                                </div>
                                <div class="text-blueGray-600">
                                    <i class="fas fa-university mr-2 text-lg text-blueGray-400"></i>
                                    University: {{ $data['speaker_data']->institute }}
                                </div>
                            </div>
                                ...
                                @if($data['speaker_data']->about != '')
                                    <div class="flex flex-wrap justify-center">
                                        <div class="w-full lg:w-9/12 px-4">
                                            <p x-data="{ isCollapsed: false, maxLength: 150, originalContent: '', content: '' }" x-init="originalContent = $el.firstElementChild.textContent.trim(); content = originalContent.slice(0, maxLength)" class="mb-4 text-base leading-relaxed text-blueGray-700">
                                                <span x-text="isCollapsed ? originalContent : content" class="text-justify block text-gray-500">{{ $data['speaker_data']->about }}</span><br>
                                                <button @click="isCollapsed = !isCollapsed" x-show="originalContent.length > maxLength"  x-text="isCollapsed ? 'Read less' : 'Read more'" class="font-normal lite-blue-color">Read more</button>
                                            </p>
                                        </div>
                                    </div>
                                @endif
                                    ...
                                    <div class="md:grid md:grid-cols-2 hover:bg-gray-50 md:space-y-0 space-y-1 p-4 border-b">
                                        ...
                                        <p>
                                            @foreach($data['credentials'] as $credential)
                                                <a href="#"><span class="inline-flex bg-blue-100 lite-blue-color h-6 px-3 justify-center items-center text- hover:text-white hover:bg-blue-500 transition-all duration-300">{{ $credential }}</span></a>
                                            @endforeach
                                        </p>
                                    </div>
                                @endif
