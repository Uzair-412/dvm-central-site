@extends('backend.layouts.app')
@section('title', $data['p_heading'])
@section('content')
    {!! Form::model($data['package'], ['method' => 'PATCH', 'route' => ['admin.packages.update', $data['package']->id], 'files' => true]) !!}
        <x-backend.card>
            <x-slot name="header">
                {{ $data['p_heading'] }}
            </x-slot>
            <x-slot name="headerActions">
                <x-utils.link class="card-header-action" :href="route('admin.packages.index')" :text="__('Cancel')" />
            </x-slot>
            <x-slot name="body">
                @include('backend.packages._form')
            </x-slot>
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Update Package')</button>
            </x-slot>
        </x-backend.card>
        {!! Form::hidden('id', $data['package']->id) !!}
    {!! Form::close() !!}
@stop